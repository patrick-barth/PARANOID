...
if ( params.domain == 'pro' || params.map_to_transcripts == true){
	...
	process mapping_STAR{
		...
		"""
		STAR --runThreadN ${task.cpus} --genomeLoad LoadAndRemove --limitBAMsortRAM ${task.memory} --genomeDir ${indexDir} --readFilesIn ${query} --outFileNamePrefix ${query.baseName}. --alignEndsType Extend5pOfRead1 --outSAMtype BAM SortedByCoordinate
		"""
	}
}
